      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12">
        <div className="flex justify-between items-center h-24">
          ...
          <div className="flex-shrink-0">
            <Link to="/">
              <img src="src/assets/image/logo.png" alt="Calesta Logo" className="h-16 w-auto cursor-pointer" />
            </Link>
          </div>
              ...
              <AnimatePresence>
                {isUserDropdownOpen && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: 15 }}
                    className="absolute right-0 mt-4 w-60 bg-white border border-gray-100 shadow-2xl rounded-xl overflow-hidden py-2 z-[60]"
                  >
                    {user ? (
                      <>
                        ...
                        {/* Condition for Admin vs User */}
                        {user.role === 'admin' ? (
                          <Link 
                            to="/dashboard" 
                            onClick={() => setIsUserDropdownOpen(false)}
                            className="flex items-center px-5 py-3 text-sm text-gray-700 hover:bg-pink-50 hover:text-pink-600 transition-all group font-bold"
                          >
                        ...
                        <div className="border-t border-gray-50 mt-1">
                          <button onClick={handleLogout} className="flex items-center w-full px-5 py-3 text-sm text-red-500 hover:bg-red-50 transition-all font-medium">
                      ...
                      <>
                        <Link to="/login" onClick={() => setIsUserDropdownOpen(false)} className="flex items-center px-5 py-3 text-sm text-gray-700 hover:bg-pink-50 hover:text-pink-600 transition-all group">
                          <LogIn className="w-4 h-4 mr-3 text-gray-400 group-hover:text-pink-500" /> Login
                        </Link>
                        <Link to="/register" onClick={() => setIsUserDropdownOpen(false)} className="flex items-center px-5 py-3 text-sm text-gray-700 hover:bg-pink-50 hover:text-pink-600 transition-all group">
                          <UserPlus className="w-4 h-4 mr-3 text-gray-400 group-hover:text-pink-500" /> Register
                        </Link>
                      </>
                    )}
                  </motion.div>
        ...
        {isMobileMenuOpen && (
          ...
          >
            ...
            <div className="space-y-6 overflow-y-auto max-h-[70vh]">
              {navLinks.map((link) => (
                <div key={link.name}>
                  <NavLink to={link.path} onClick={() => !link.hasDropdown && setIsMobileMenuOpen(false)} className={({ isActive }) => `text-xl font-bold border-b border-gray-100 pb-2 mb-2 block w-full ${isActive ? 'text-pink-600' : 'text-gray-900'}`}>{link.name}</NavLink>
                  {link.hasDropdown && (
                ...
                {user ? (
                   <>
                     <div className="flex items-center space-x-3 mb-2">
                        ...
                        <div>
                          <p className="text-sm text-gray-400">Hello, {user.role === 'admin' ? '(Admin)' : ''}</p>
                          <p className="text-lg font-bold text-gray-900">{user.name}</p>
                        </div>
                     </div>
                     {user.role === 'admin' ? (
                       <Link to="/dashboard" onClick={() => setIsMobileMenuOpen(false)} className="text-lg text-pink-600 font-bold">Admin Dashboard</Link>
                     ) : (
                       <>
